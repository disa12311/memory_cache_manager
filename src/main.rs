...
impl CacheManager {
    ...
    fn get_ram_cache_fast(&self) -> f32 {
        ...
        // Chỉ dùng wmic (nhanh hơn PowerShell)
        if let Ok(output) = Command::new("wmic")
            .creation_flags(CREATE_NO_WINDOW)
            ...
            if let Ok(text) = String::from_utf8(output.stdout) {
                for line in text.lines() {
                    if let Ok(kb) = line.trim().parse::<u64>() {
                        ...
                        return used_mb * 0.6; // Ước tính 60% RAM đang dùng là cache
                    }
                }
            }
        }
    ...
    fn clean_ram_cache(&mut self) {
        ...
        // Chỉ dùng 1 method hiệu quả nhất: Empty Working Sets
...
impl eframe::App for CacheManager {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        // CHỈ update RAM mỗi 3 giây để giảm lag
        ...
        // Chỉ repaint mỗi 3 giây thay vì 1 giây
